...
impl fmt::Debug for Command<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Command::SendOpCond(arg) => write!(f, "SendOpCond({arg})"),
            Command::AllSendCid => write!(f, "AllSendCid"),
            Command::SendRelativeAddr => write!(f, "SendRelativeAddr"),
            Command::SwitchFunc(arg, _) => write!(f, "SwitchFunc({arg})"),
            ...
            Command::SdStatus(_) => write!(f, "SdStatus"),
            Command::SetBlockLen(len) => write!(f, "SetBlockLen({len})"),
...
impl<'a> Command<'a> {
    fn cmd_index(&self) -> u8 {
        match self {
            ...
            Command::SdStatus(_) => 13,
            Command::SetBlockLen(_) => 16,
    ...
    pub(crate) fn build(self) -> (Cmd, u32, Option<DataXfer<'a>>) {
        ...
        match self {
            ...
            Command::SendOpCond(arg) => (cmd, arg, None), // R3 response, no CRC check
            ...
            Command::SendStatus(arg) => (cmd_crc, arg, None),
            Command::SdStatus(buf) => (
                cmd_crc.with_data_expected(true),
                0,
                Some(DataXfer::Read(buf)),
            ),
            Command::SetBlockLen(arg) => (cmd_crc, arg, None),
            ...
            Command::WriteMultipleBlock(block, buf) => (
                cmd_crc.with_data_expected(true).with_read_write(true).with_send_auto_stop(true),
                block,
                Some(DataXfer::Write(buf)),
            ),
